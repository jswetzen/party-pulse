...
that question -- or, at the same score's low end, how unusually close they are (see
PLAN.md "Interesting stats suggestions"). This is a decision aid only: it never reveals
anything itself, it only ranks candidates for the host to review and load into the
existing screen_control form (see the `prefill` handling in views.screen_control) --
still requires the host's own explicit "show"/"reveal" click, same as PLAN.md's "Reveal
semantics" for the panel it sits next to.

Scoring is a pluggable strategy (PLAN.md decision: keep this normalized effect-size
measure and a future simple "% gap" measure both available and swappable without
restructuring). See SCORING_STRATEGIES at the bottom -- add a sibling scoring function
with the same signature and register it there to add a mode; nothing else changes.
"""
...
# A group below this size still appears in the ranked list -- PLAN.md's anonymity model
...
# How many top-ranked findings to surface. Recomputed on every host-console page load (no
# caching, no background job) -- see compute_suggestions()'s docstring for why that's
# cheap enough at this app's actual scale (PLAN.md: 39 seeded questions, wedding-guest-list
# response counts).
TOP_N = 10
...
def _sv_number(value: float, decimals: int = 1) -> str:
    """Swedish decimal-comma formatting for numbers embedded in blurb text. Not a
...
def compute_suggestions(scoring: str = "effect_size", top_n: int = TOP_N) -> list[Suggestion]:
    """Global scan: every live, non-system question x every non-overall breakdown x every
    group, scored and ranked by |score| descending (most extreme first -- see module
    docstring and PLAN.md; a single list, not separate "divergent" and "similar" lists).
    ...
    page load, each just arithmetic over a question's already-fetched Response rows (one
    query per question, reused across all 4 breakdowns via aggregations._group). Confirmed
    against this repo's actual seeded question count and test fixtures, not assumed."""
...
# ---------------------------------------------------------------------------
# Scoring strategy: normalized effect size (the live default, PLAN.md decision).
...
# Registry of swappable scoring strategies, keyed by name and passed to
# compute_suggestions(scoring=...). "effect_size" is the live default per PLAN.md's
# decision; a future "simple_gap" strategy (raw percentage-point / unit gap, no
# normalization) can be added as a sibling function with the same
# (question, group_label, overall_values, group_values) -> (score, blurb) | None
# signature and registered here -- compute_suggestions() itself needs no changes.
SCORING_STRATEGIES = {
